...
behave more like ido (ido + flx-ido + ido-vertical-mode). Because the guide inlcudes
many other snippets now which I consider useful in general it has become more of collection
of snippets to improve helms default interface and is not targeted to former ido users only. 
...
Everyone has his own opinions and although I consider the follwoing snippets improvements
you might have a different opinion. You don't have to follow this guide completely because
I have splitted it into parts and each snippet should be independent from each other.

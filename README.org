* Better Helm
:PROPERTIES:
:SUMMARY: Opinionated configuration guide for the helm package of Emacs
:END:

This guide was intended for people which used ido in the past and want helm to
behave more like ido (ido + flx-ido + ido-vertical-mode). Because now there are many other
improvements which are useful in general and not only for former ido users
 it is has become more like a general guide for people who don't like the default interface 
of helm.
...
- Remove the annoying dots for current and parent directory in helm file navigation
...
I included the references to the origins.
